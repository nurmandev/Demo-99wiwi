  ...
  io.of("/coinflip").on("connection", socket => {
    ...
    socket.on(
      ...
      async (players, color, betAmount, private, costMultiplier) => {
        ...
        const minBetAmount = Number(process.env.COINFLIP_MIN_BET) || 10;
        const maxBetAmount = Number(process.env.COINFLIP_MAX_BET) || 1000;
        if (
        ...
        try {
          ...
          if (dbUser.selfExcludes.coinflip > Date.now()) {
            return socket.emit(
              ...
              `You have self-excluded yourself for another ${(
                (dbUser.selfExcludes.coinflip - Date.now()) /
                3600000
              ).toFixed(1)} hours.`
            );
          }
          ...
          if (private) {
            ...
            const feePercentage =
              parseFloat(process.env.COINFLIP_FEE_PERCENTAGE) || 0.05; // Default to 5% if not set
            const houseEdge = parseFloat(betAmount.toFixed(2)) * feePercentage;
    ...
    socket.on("join-game", async (gameId, color, recaptchaResponse) => {
      ...
      try {
        ...
        if (game.privateGame) {
          ...
          const feePercentage =
            parseFloat(process.env.COINFLIP_FEE_PERCENTAGE) || 0.05; // Default to 5% if not set
          const houseEdge = betAmount * feePercentage;
          ...
          const feePercentage =
            parseFloat(process.env.COINFLIP_FEE_PERCENTAGE) || 0.05; // Default to 5% if not set
          const houseEdge = game.betAmount * feePercentage;
          // Apply user's rakeback if eligible
          await checkAndApplyRakeback(user.id, houseEdge);
        ...
        if (shouldRoll) {
          ...
          const winner = newPlayers.find(
            player => player.color === winningSide
          );
