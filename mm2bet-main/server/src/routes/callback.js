...
router.post("/", async (req, res, next) => {
  ...
  const apiSecretKey = process.env.OAXPAY_MERCHANT_ID;
  ...
  if (calculatedHmac === hmacHeader) {
    ...
    if (data.status == "Paid") {
      ...
      await User.updateOne(
        { _id: user._id },
        { $inc: { wallet: siteValue, totalDeposited: siteValue } }
      );
    } else {
      console.log(
        colors.blue(
          "0xapay >> Deposit pending! " +
            data.amount +
            " " +
            data.currency +
            "."
