  ...
  io.of("/cases").on("connection", socket => {
    ...
    socket.on("cases:open", async slug => {
      try {
        ...
        if (!slug) return socket.emit("cases:error", "Not a valid case slug!");
        ...
        if (dbUser.wallet < parseFloat(caseData.price.toFixed(2))) {
          return socket.emit(
            "cases:error",
            "You can't afford to open this case!"
          );
        }
        ...
        insertNewWalletTransaction(
          user._id,
          -Math.abs(parseFloat(caseData.price.toFixed(2))),
          "Cases open",
          { SoloCaseSlug: caseData.slug }
        );
        ...
        await User.updateOne(
          { _id: user.id },
          {
            $inc: {
              wallet: +Math.abs(
                parseFloat(newGame.caseResult.item.price.toFixed(2))
              ),
            },
          }
        );
        insertNewWalletTransaction(
          user._id,
          +Math.abs(parseFloat(newGame.caseResult.item.price.toFixed(2))),
          "Cases win",
          { SoloCase: newGame._id }
        );
