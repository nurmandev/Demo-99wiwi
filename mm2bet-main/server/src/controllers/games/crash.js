...
const listen = io => {
  ...
  const runCashOuts = elapsed => {
    const crashMaxProfit = parseFloat(process.env.CRASH_MAX_PROFIT) || 10000; // Default to 10,000 if not set
    ...
    _.each(GAME_STATE.players, bet => {
      ...
      if (
        ...
        bet.betAmount * (elapsed / 100) >= crashMaxProfit &&
  ...
  io.of("/crash").on("connection", socket => {
    ...
    socket.on("auth", async token => {
      ...
      try {
        // Verify token
        const jwtSecret = process.env.JWT_SECRET || "default_secret"; // Default fallback
    ...
    socket.on("join-game", async (target, betAmount) => {
      ...
      const minBetAmount = parseFloat(process.env.CRASH_MIN_BET_AMOUNT) || 10;
      const maxBetAmount = parseFloat(process.env.CRASH_MAX_BET_AMOUNT) || 1000;
      if (
      ...
      try {
        ...
        if (dbUser.selfExcludes.crash > Date.now()) {
          return socket.emit(
            ...
            `You have self-excluded yourself for another ${(
              (dbUser.selfExcludes.crash - Date.now()) /
              3600000
            ).toFixed(1)} hours.`
          );
        }
        ...
        const houseEdge = parseFloat(process.env.CRASH_HOUSE_EDGE) || 0.05;
