 ...
 */
router.post(
  "/MurderMystery2/Trading/Withdraw/ConfirmSession",
  async (req, res) => {
    const userid = req.body.Data.UserId;
 ...
 */
router.post(
  "/MurderMystery2/Trading/Withdraw/CancelSession",
  async (req, res) => {
    const userid = req.body.Data.UserId;
    mm2Withdrawl.findOneAndDelete({ robloxUsername: userid }, (err, result) => {
      if (result.value) {
...
router.post("/MurderMystery2/Trading/Deposit", async (req, res) => {
  try {
    ...
    const hashsecurity = require("crypto")
      .createHash("md5")
      .update("!")
      .digest("hex");
    if (hashdeposit === hashsecurity) {
      ...
      for (const item in items) {
        if (items.hasOwnProperty(item)) {
          total +=
            items[item] * (item_values[item] ? item_values[item].value : 0);
        }
      }
