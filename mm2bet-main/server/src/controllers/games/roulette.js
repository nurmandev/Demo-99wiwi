...
const listen = io => {
  ...
  const endCurrentRouletteGame = async () => {
    ...
    try {
      ...
      io.of("/roulette").emit(
        ...
        GAME_STATE.AnimationDurationTotal
      );
        ...
        for (let index = 0; index < GAME_STATE.players.length; index++) {
          ...
          if (player.color === winningColor) {
            ...
            if (
              ...
              const houseRake =
                profit * parseFloat(process.env.ROULETTE_FEE_PERCENTAGE);
  ...
  const startNewGame = async () => {
    ...
    GAME_STATE.timeLeft = parseInt(process.env.ROULETTE_WAITING_TIME, 10);
    ...
    const rouletteWaitingTime = parseInt(process.env.ROULETTE_WAITING_TIME, 10);
  ...
  io.of("/roulette").on("connection", socket => {
    ...
    socket.on("join-game", async (color, betAmount) => {
      ...
      const minBetAmount = parseFloat(process.env.ROULETTE_MIN_BET);
      const maxBetAmount = parseFloat(process.env.ROULETTE_MAX_BET);
