...
const listen = io => {
  ...
  const payoutWinnerLow = async () => {
    try {
      ...
      const profit = GAME_STATE.players
        ...
        .reduce((a, b) => a + b, 0);
      const houseRake = profit * process.env.JACKPOT_FEE_PERCENTAGE;
      const feeMultiplier = 1 - process.env.JACKPOT_FEE_PERCENTAGE;
  ...
  const EndGameLow = async () => {
    ...
    const maxTicket =
      GAME_STATE.players[GAME_STATE.players.length - 1].tickets.max;
    ...
    io.of("/jackpot").emit(
      "game-rolled-low",
      GAME_STATE.winningTicketCalc,
      GAME_STATE.maxTicketCalc,
      GAME_STATE.AnimationDuration
    );
    ...
    setTimeout(() => {
      ...
      GAME_STATE.playerWinnerIndex = GAME_STATE.players.findIndex(
        player => player._id === GAME_STATE.winner._id
      );
      io.of("/jackpot").emit(
        "pottitlehigh-updated-low",
        GAME_STATE.AnimationEndedStatus,
        GAME_STATE.playerWinnerIndex
      );
    }, 11500);
  ...
  const EndGameMiddle = async () => {
    ...
    const maxTicket =
      GAME_STATE_MIDDLE.players[GAME_STATE_MIDDLE.players.length - 1].tickets
        .max;
    ...
    io.of("/jackpot").emit(
      "game-rolled-middle",
      GAME_STATE_MIDDLE.winningTicketCalc,
      GAME_STATE_MIDDLE.maxTicketCalc,
      GAME_STATE_MIDDLE.AnimationDuration
    );
    ...
    setTimeout(() => {
      ...
      GAME_STATE_MIDDLE.playerWinnerIndex = GAME_STATE_MIDDLE.players.findIndex(
        player => player._id === GAME_STATE_MIDDLE.winner._id
      );
      io.of("/jackpot").emit(
        "pottitlehigh-updated-middle",
        GAME_STATE_MIDDLE.AnimationEndedStatus,
        GAME_STATE_MIDDLE.playerWinnerIndex
      );
    }, 11500);
  ...
  const EndGameHigh = async () => {
    ...
    const maxTicket =
      GAME_STATE_HIGH.players[GAME_STATE_HIGH.players.length - 1].tickets.max;
    ...
    io.of("/jackpot").emit(
      "game-rolled-high",
      GAME_STATE_HIGH.winningTicketCalc,
      GAME_STATE_HIGH.maxTicketCalc,
      GAME_STATE_HIGH.AnimationDuration
    );
    ...
    setTimeout(() => {
      ...
      GAME_STATE_HIGH.playerWinnerIndex = GAME_STATE_HIGH.players.findIndex(
        player => player._id === GAME_STATE_HIGH.winner._id
      );
      io.of("/jackpot").emit(
        "pottitlehigh-updated-high",
        GAME_STATE_HIGH.AnimationEndedStatus,
        GAME_STATE_HIGH.playerWinnerIndex
      );
    }, 11500);
  ...
  const endCurrentJackpotGameMiddle = async () => {
    ...
    // Get max ticket in the game
    const maxTicket =
      GAME_STATE_MIDDLE.players[GAME_STATE_MIDDLE.players.length - 1];
  ...
  const endCurrentJackpotGameHigh = async () => {
    ...
    // Get max ticket in the game
    const maxTicket =
      GAME_STATE_HIGH.players[GAME_STATE_HIGH.players.length - 1];
  ...
  const addPlayerToCurrentGameLow = async player => {
    ...
    if (GAME_STATE.players.length === 2) {
      ...
      io.of("/jackpot").emit(
        ...
        GAME_STATE.timeLeft,
        GAME_STATE.pieStartAngle,
        GAME_STATE.pieEndAngle
      );
  ...
  const addPlayerToCurrentGameMiddle = async player => {
    ...
    if (GAME_STATE_MIDDLE.players.length === 2) {
      ...
      io.of("/jackpot").emit(
        ...
        GAME_STATE_MIDDLE.timeLeft,
        GAME_STATE_MIDDLE.pieStartAngle,
        GAME_STATE_MIDDLE.pieEndAngle
      );
  ...
  const addPlayerToCurrentGameHigh = async player => {
    ...
    if (GAME_STATE_HIGH.players.length === 2) {
      ...
      io.of("/jackpot").emit(
        ...
        GAME_STATE_HIGH.timeLeft,
        GAME_STATE_HIGH.pieStartAngle,
        GAME_STATE_HIGH.pieEndAngle
      );
  ...
  const startNewGameLow = async () => {
    ...
    GAME_STATE.timeLeft = Number(process.env.JACKPOT_WAITING_TIME) || 30; // Default to 30 if not set
  ...
  const startNewGameMiddle = async () => {
     ...
     * (5 - Ended)
     */
    GAME_STATE_MIDDLE.timeLeft = Number(process.env.JACKPOT_WAITING_TIME) || 30; // Default to 30 if not set
  ...
  const startNewGameHigh = async () => {
     ...
     * (5 - Ended)
     */
    GAME_STATE_HIGH.timeLeft = Number(process.env.JACKPOT_WAITING_TIME) || 30; // Default to 30 if not set
  ...
  io.of("/jackpot").on("connection", socket => {
    ...
    socket.on("join-game-low", async betAmount => {
      ...
      const minBetAmountLow = Number(process.env.MIN_BET_AMOUNT_LOW) || 10;
      const maxBetAmountLow = Number(process.env.MAX_BET_AMOUNT_LOW) || 1000;
      if (
        !GAME_STATE.players.map(player => player._id).includes(user.id) &&
        (parseFloat(betAmount.toFixed(2)) < minBetAmountLow ||
          parseFloat(betAmount.toFixed(2)) > maxBetAmountLow)
      ) {
        return socket.emit(
          "game-join-error-low",
      ...
      if (GAME_STATE.players.map(player => player._id).includes(user.id)) {
        try {
          ...
          if (dbUser.selfExcludes.jackpot > Date.now()) {
            return socket.emit(
              "game-join-error-low",
              `You have self-excluded yourself for another ${(
                (dbUser.selfExcludes.jackpot - Date.now()) /
                3600000
              ).toFixed(1)} hours.`
            );
          }
          ...
          let player_indexCheck = GAME_STATE.players.findIndex(
            p => p._id === user.id
          );

          if (
            GAME_STATE.players[player_indexCheck].betAmount +
              parseFloat(betAmount.toFixed(2)) >
            maxBetAmountLow
          ) {
            return socket.emit(
              "game-join-error-low",
              `Your bet will reach more than the max. bet amount allowed on this pot.`
            );
          }
          ...
          const exisitingEntry = GAME_STATE.players
            .map(player => player._id)
            .includes(user.id);

          if (exisitingEntry) {
            let player_index = GAME_STATE.players.findIndex(
              p => p._id === user.id
            );

            GAME_STATE.players[player_index].betAmount =
              GAME_STATE.players[player_index].betAmount +
              parseFloat(betAmount.toFixed(2));
            ...
            GAME_STATE.players.forEach((player, i) => {
              if (i === 0) {
                ...
                let maxTicket2 =
                  GAME_STATE.players[lastPlayer].tickets.max +
                  1 +
            ...
            GAME_STATE.players[player_index].winningPercentage =
              100 -
              ((totalValue - GAME_STATE.players[player_index].betAmount) /
                totalValue) *
                100;
        ...
        try {
          ...
          if (GAME_STATE.players.length === 45) {
            return socket.emit(
              "game-join-error-low",
              "Low pot reached max. of 45 players."
            );
          }
    ...
    socket.on("join-game-middle", async betAmount => {
      ...
      const minBetAmountMid = Number(process.env.MIN_BET_AMOUNT_MID) || 50;
      const maxBetAmountMid = Number(process.env.MAX_BET_AMOUNT_MID) || 5000;
      if (
        !GAME_STATE_MIDDLE.players
          .map(player => player._id)
          .includes(user.id) &&
        (parseFloat(betAmount.toFixed(2)) < minBetAmountMid ||
          parseFloat(betAmount.toFixed(2)) > maxBetAmountMid)
      ) {
        return socket.emit(
          "game-join-error-middle",
      ...
      // If user has already joined this game
      if (
        GAME_STATE_MIDDLE.players.map(player => player._id).includes(user.id)
      ) {
        try {
          // Get user from database
          const dbUser = await User.findOne({ _id: user.id });
          ...
          let player_indexCheck = GAME_STATE_MIDDLE.players.findIndex(
            p => p._id === user.id
          );

          if (
            GAME_STATE_MIDDLE.players[player_indexCheck].betAmount +
              parseFloat(betAmount.toFixed(2)) >
            maxBetAmountMid
          ) {
            return socket.emit(
          ...
          // Get exising user entry
          const exisitingEntry = GAME_STATE_MIDDLE.players
            .map(player => player._id)
            .includes(user.id);

          if (exisitingEntry) {
            let player_index = GAME_STATE_MIDDLE.players.findIndex(
              p => p._id === user.id
            );

            GAME_STATE_MIDDLE.players[player_index].betAmount =
              GAME_STATE_MIDDLE.players[player_index].betAmount +
              parseFloat(betAmount.toFixed(2));
            ...
            GAME_STATE_MIDDLE.players.forEach((player, i) => {
              if (i === 0) {
                ...
                let lastPlayer = i - 1;
                let minTicket2 =
                  GAME_STATE_MIDDLE.players[lastPlayer].tickets.max + 1;
                let maxTicket2 =
                  GAME_STATE_MIDDLE.players[lastPlayer].tickets.max +
                  1 +
                  player.betAmount * 100 -
                  1;
        ...
        try {
          ...
          if (GAME_STATE_MIDDLE.players.length === 45) {
            return socket.emit(
              "game-join-error-middle",
              "Medium pot reached max. of 45 players."
            );
          }
    ...
    socket.on("join-game-high", async betAmount => {
      ...
      // More validation on the bet value
      const minBetAmountHigh = Number(process.env.MIN_BET_AMOUNT_HIGH) || 100;
      const maxBetAmountHigh = Number(process.env.MAX_BET_AMOUNT_HIGH) || 10000;
      if (
        !GAME_STATE_HIGH.players.map(player => player._id).includes(user.id) &&
        (parseFloat(betAmount.toFixed(2)) < minBetAmountHigh ||
          parseFloat(betAmount.toFixed(2)) > maxBetAmountHigh)
      ) {
        return socket.emit(
          "game-join-error-high",
      ...
      if (GAME_STATE_HIGH.players.map(player => player._id).includes(user.id)) {
        try {
          ...
          let player_indexCheck = GAME_STATE_HIGH.players.findIndex(
            p => p._id === user.id
          );

          if (
            GAME_STATE_HIGH.players[player_indexCheck].betAmount +
              parseFloat(betAmount.toFixed(2)) >
            maxBetAmountHigh
          ) {
            return socket.emit(
          ...
          // Get exising user entry
          const exisitingEntry = GAME_STATE_HIGH.players
            .map(player => player._id)
            .includes(user.id);

          if (exisitingEntry) {
            let player_index = GAME_STATE_HIGH.players.findIndex(
              p => p._id === user.id
            );

            GAME_STATE_HIGH.players[player_index].betAmount =
              GAME_STATE_HIGH.players[player_index].betAmount +
              parseFloat(betAmount.toFixed(2));
            ...
            GAME_STATE_HIGH.players.forEach((player, i) => {
              if (i === 0) {
                ...
                let lastPlayer = i - 1;
                let minTicket2 =
                  GAME_STATE_HIGH.players[lastPlayer].tickets.max + 1;
                let maxTicket2 =
                  GAME_STATE_HIGH.players[lastPlayer].tickets.max +
                  1 +
                  player.betAmount * 100 -
                  1;
            ...
            GAME_STATE_HIGH.players[player_index].winningPercentage =
              100 -
              ((totalValue - GAME_STATE_HIGH.players[player_index].betAmount) /
                totalValue) *
                100;
        ...
        try {
          ...
          if (GAME_STATE_HIGH.players.length === 45) {
            return socket.emit(
              "game-join-error-high",
              "High pot reached max. of 45 players."
            );
          }
