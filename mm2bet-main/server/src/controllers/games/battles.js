...
const listen = async io => {
  ...
  const generateCaseResult = async (
    caseObj,
    hash,
    playerCount,
    roundNum,
  ...
  const runGame = async battleId => {
    const pendingGameIndex = PENDING_GAMES.findIndex(
      game => String(game._id) === String(battleId)
    );
    ...
    for (let i = 0; i < battle.cases.length; i++) {
      ...
      io.of("/battles")
        ...
        .emit("battles:round", {
          battleId: battleId,
          result: caseResult,
          img: getRandomWeightedItems(
            caseList.find(caseItem => caseItem.slug === battle.cases[i].slug),
            33
          ),
        });
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (isEqual) {
          ...
          insertNewWalletTransaction(
            battle.players[i].id,
            +Math.abs(parseFloat(winAmount.toFixed(2))),
            "Battles win",
            { battlesGameId: battle._id }
          );
          io.of("/battles")
            .to(battle.players[i].id)
            .emit("update-wallet", +Math.abs(parseFloat(winAmount.toFixed(2))));
        } else {
          if (winningTeam == 1 && i > 1) continue;
          if (winningTeam == 2 && i <= 1) continue;
          await User.updateOne(
            { _id: battle.players[i].id },
            { $inc: { wallet: +Math.abs(parseFloat(winAmount.toFixed(2))) } }
          );
          insertNewWalletTransaction(
            battle.players[i].id,
            +Math.abs(parseFloat(winAmount.toFixed(2))),
  ...
  io.of("/battles").on("connection", socket => {
    ...
    socket.on(
      ...
      async (
        ...
        if (
          ...
          return socket.emit(
            "battles:error",
            "Not a valid gamemode! If you continue to try and break the code, you will be ip blacklisted."
          );
        ...
        for (const item of selectedCases) {
          ...
          if (!last)
            return socket.emit(
              "battles:error",
              "Not a valid case! If you continue to try and break the code, you will be ip blacklisted."
            );
        ...
        try {
          ...
          if (dbUser.wallet < parseFloat(totalCost.toFixed(2))) {
            return socket.emit(
              "battles:error",
              "You can't afford to create this battle!"
            );
          }
          const gameTypeInt =
            selectedMode == "1v1"
          ...
          insertNewWalletTransaction(
            user.id,
            -Math.abs(parseFloat(totalCost.toFixed(2))),
            "Battles creation",
          ...
          io.of("/battles")
            .to(user.id)
            .emit("update-wallet", -Math.abs(parseFloat(totalCost.toFixed(2))));
          ...
          const houseEdge = parseFloat(process.env.CRASH_HOUSE_EDGE) || 0.05; // Default to 5% if not set
          const houseRake = parseFloat(betAmount.toFixed(2)) * houseEdge;
    ...
    socket.on("battles:reqdata", async id => {
      try {
        if (!id) return socket.emit("battles:error", "Not a valid battle id!");

        const gameData = PENDING_GAMES.find(game => String(game._id) === id)
          ? PENDING_GAMES.find(game => String(game._id) === id)
          : await BattlesGame.findOne({ _id: id });
    ...
    socket.on("battles:join", async battleId => {
      try {
        ...
        const pendingGame = PENDING_GAMES.find(
          game => String(game._id) === String(battleId)
        );
        let battle;
        ...
        if (pendingGame) {
          ...
          if (!battle) {
            return socket.emit(
              "battles:error",
              "The game you are trying to join is invalid!"
            );
          }
        }
        ...
        if (betAmount > user.wallet)
          return socket.emit(
            "battles:error",
            "You can't afford to join this game!"
          );
        ...
        io.of("/battles")
          .to(user.id)
          .emit("update-wallet", -Math.abs(parseFloat(betAmount.toFixed(2))));
        ...
        let newPlayers = [...battle.players, player];

        await BattlesGame.findOneAndUpdate(
          { _id: battleId },
          { $set: { players: [...newPlayers] } }
        );

        const index = PENDING_GAMES.findIndex(
          game => String(game._id) === String(battleId)
        );
    ...
    socket.on("battles:callbot", async battleId => {
      try {
        ...
        let newPlayers = [...battle.players, ...bots];
