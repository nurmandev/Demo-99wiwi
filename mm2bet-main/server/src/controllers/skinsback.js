...
const WITHDRAW_COMMISSION = parseFloat(process.env.WITHDRAW_FEE) || 0.05; // Default to 5% if not set
...
const withdraw_itemsCSGO = async (user_id, items, tradelink) => {
  try {
    ...
    if (dbUser.wagerNeededForWithdraw > 0)
      return {
        success: false,
        error: `You must wager atleast $${dbUser.wagerNeededForWithdraw.toFixed(
          2
        )} before withdrawing!`,
      };
  ...
  try {
    ...
    let items_filtered = res.items
      .filter(i => {
        return (
          i.count > 0 &&
          i.price > config.authentication.skinsback.withdrawMinItemPrice
        );
      })
      .map(i => {
        ...
        let price = parseFloat(
          parseFloat(i.price + i.price * (better_price / 100)).toFixed(2)
        );
    ...
    console.log(
      `[SKINSBACK SHOP] Loaded ${items_filtered.length} CS:GO items from the API for user ${user_id}.`
    );
...
const load_market_items_RUST = async user_id => {
  try {
    ...
    console.log(
      `[SKINSBACK SHOP] Loaded ${items_filtered.length} RUST items from the API for user ${user_id}.`
    );
...
const load_market_items_DOTA2 = async user_id => {
  try {
    ...
    console.log(
      `[SKINSBACK SHOP] Loaded ${items_filtered.length} DOTA2 items from the API for user ${user_id}.`
    );
