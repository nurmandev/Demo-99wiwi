  ...
  try {
    ...
    if (activeRace) {
      ...
      return res.json({
        ...
        prizeDistribution: process.env.RACE_PRIZE_DISTRIBUTION
...
router.get("/last", async (req, res, next) => {
  try {
    ...
    const lastRace = await Race.find({ active: false })
      .sort({ endingDate: -1 })
      .limit(1);
...
router.get("/me", validateJWT, async (req, res, next) => {
  try {
    ...
    if (activeRace) {
      ...
      const allEntries = await RaceEntry.find({ _race: activeRace.id }).sort({
        value: -1,
      });
      ...
      return res.json({
        ...
        myPosition: existingEntry
          ? allEntries.map(entry => String(entry._user)).indexOf(req.user.id) +
            1
          : -1,
