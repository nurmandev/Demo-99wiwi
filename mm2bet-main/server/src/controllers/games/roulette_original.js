...
const ROULETTE_ORDER = [
  3, 2, 5, 20, 5, 2, 3, 2, 3, 2, 3, 2, 5, 2, 5, 2, 3, 2, 3, 2, 3, 2, 5, 2, 5, 2,
  3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 5, 2, 5, 2, 3, 2, 3, 2, 3, 2, 5, 2, 5, 2, 3, 2,
  3, 2,
];
...
const getCurrentGame = () => ({
  ...GAME_STATE,
  rollStart: GAME_STATE.rollStatus
    ? GAME_STATE.rollStatus.rollStart - +new Date()
    : null,
  privateSeed: null,
  intervalId: null,
});
...
const listen = io => {
            ...
            const timeout = setTimeout(async () => {
              ...
              for (let index = 0; index < GAME_STATE.players.length; index++) {
                ...
                if (player.color === secondWinningColor) {
                  ...
                  // Get the fee percentage from environment variables
                  const feePercentage = process.env.ROULETTE_FEE_PERCENTAGE
                    ? parseFloat(process.env.ROULETTE_FEE_PERCENTAGE)
                    : 0.05;
  ...
  const startNewGame = async () => {
    ...
    const waitingTime = process.env.ROULETTE_WAITING_TIME
      ? parseInt(process.env.ROULETTE_WAITING_TIME, 10)
      : 30; // Default fallback

    GAME_STATE.timeLeft = waitingTime;
  ...
  io.of("/roulette").on("connection", socket => {
    ...
    socket.on("join-game", async (color, betAmount) => {
      ...
      const minBetAmount = process.env.ROULETTE_MIN_BET;
      const maxBetAmount = process.env.ROULETTE_MAX_BET;
      if (
        ...
        if (dbUser.selfExcludes.roulette > Date.now()) {
          return socket.emit(
            ...
            `You have self-excluded yourself for another ${(
              (dbUser.selfExcludes.roulette - Date.now()) /
              3600000
            ).toFixed(1)} hours.`
          );
        }
